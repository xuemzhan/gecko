...
def scan_files(source_paths, output_file, extensions=None):
    """
    扫描指定目录或文件列表中的指定格式文件,并将内容整合到一个文件中
    
    参数:
        source_paths: 要扫描的源目录或文件路径列表
        output_file: 输出文件路径
        extensions: 要扫描的文件扩展名列表,如 ['.py', '.txt']。默认为 ['.py']
    """
    # 设置默认扩展名
    ...
    with open(output_file, 'w', encoding='utf-8') as out_f:
        for index, file_path in enumerate(target_files, start=1):
            # 计算相对于当前工作目录的路径
            try:
                display_path = file_path.relative_to(current_dir)
            except ValueError:
                # 如果无法计算相对路径，使用绝对路径
    ...
    print(f"成功! 共处理 {len(target_files)} 个文件")
...
def main():
    ...
    # 获取文件扩展名
    print("\n请输入要扫描的文件扩展名 (多个扩展名用空格、逗号或分号分隔)")
    print("例如: .py .txt .md 或 py, txt, md")
    extensions_input = input("留空默认扫描 Python 文件 (.py): ").strip()
    
    if extensions_input:
        # 分割扩展名
        if ',' in extensions_input:
            extensions = [e.strip() for e in extensions_input.split(',')]
        elif ';' in extensions_input:
            extensions = [e.strip() for e in extensions_input.split(';')]
        else:
        ...
        extensions = None  # 使用默认值 ['.py']
    
    # 获取输出文件名
    output_filename = input("\n请输入输出文件名 (默认: files_collection.txt): ").strip()
    
    # 设置默认输出文件名
    if not output_filename:
        output_filename = "files_collection.txt"
    
    print(f"\n开始扫描...")
    # 执行扫描
    scan_files(source_paths, output_filename, extensions)
