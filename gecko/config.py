...
class GeckoSettings(BaseSettings):
    ...
    default_model_timeout: float = Field(
        default=30.0, ge=5.0, description="LLM 请求超时(秒)，Agent 与 Engine 默认使用该值"
    )
    ...
    workflow_history_retention: int = Field(
        default=20, ge=1, description="持久化时保留的历史记录步数(防止 IO 爆炸)"
    )
    ...
    # [新增]
    memory_summary_reserve_tokens: int = Field(
        default=500, ge=0, description="为摘要预留的 Token 数"
    )
...
def get_settings(force_reload: bool = False) -> GeckoSettings:
    """
    获取全局配置实例。
    :param force_reload: 为 True 时强制从环境重新加载配置。
    """
...
def configure_settings(**overrides: Any) -> GeckoSettings:
    """
    以代码形式覆盖部分配置字段，并刷新全局实例。
    传入的 overrides 会覆盖环境变量和默认值。
    """
    global _default_settings
    _default_settings = GeckoSettings(**{k: v for k, v in overrides.items()})
    return _default_settings


def reset_settings() -> None:
    """
    清空全局配置单例，下次调用 get_settings 时会重新从环境加载。
    """
    global _default_settings
